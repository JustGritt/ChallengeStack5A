...
export default function NewService() {
  ...
  const validationSchema = Yup.object().shape({
    ...
    time: Yup.number()
      ...
      .min(30, "Time must be at least 30 minutes")
      .max(480, "Time must be at most 8 hours")
      .integer("Time must be a multiple of 30")
      .positive("Time must be a positive number")
      .test(
        "is-multiple-of-30",
        "Time must be a multiple of 30",
        (value) => value % 30 === 0
      ),
    price: Yup.number()
        ...
        <Form id="profileFields" className="flex flex-col px-4 gap-1">
          ...
          <div className="sm:grid sm:grid-cols-2 gap-4 w-full mt-4">
            <div className="flex w-full flex-col">
              <label
                htmlFor="time"
                className="text-black text-sm font-semibold inline"
              >
                Time <span className="text-red-600">*</span>
                <ErrorMessage
                  name="time"
                  component="span"
                  className="ml-4 text-red-600 leading-3 text-sm"
                />
              </label>
              <Field
                type="number"
