...
export default function DashboardMenu() {
        ...
        await fetch("https://api.odicylens.com/companies?page=0", {
          ...
          .then((data) =>
            data["hydra:member"].map((company: any) => {
              company.isValid
                ? setNotifications([...notifications, company])
                : null;
            })
          );
      }
  ...
  const fetchCompanies = useCallback(async () => {
    if (userConfig.isAdmin) {
      ...
      const invalidCompanies = data["hydra:member"].filter(
        (company: any) => !company.isValid
      );
  ...
  const navigation = [
    {
      ...
      role: ["isAdmin", "isOwner", "isWorker", "isClient"],
    },
    {
      name: "Company",
    ...
    },
    {
      ...
      icon: ShoppingCartIcon,
      current: pathname === "/dashboard/stores",
      role: ["isAdmin", "isOwner"],
    },
    {
      name: "Employees",
    ...
    },
    {
      ...
      role: ["isOwner", "isWorker", "isClient"],
    },
    {
      name: "History",
