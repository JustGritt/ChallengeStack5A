...
export default function EditProfile() {
    ...
    // Update profile
    const [profileFiledUpdated, setProfileFiledUpdated] = useState(false);
    const [profileFields, setProfileFields] = useState({} as any);
    const initialProfileValues: UserUpdateProfile = { firstname: "" };

    useEffect(() => {
        if (user && !profileFiledUpdated) {
            // fetch(`https://api.odicylens.com/users/${user?.id}`, { method: "GET" })
            //     .then((res) => res.json())
            //     .then((data) => {
            //         setProfileFields(data);
            //     });
            console.log("Fetching user profile data", user.id)
            setProfileFiledUpdated(true);
        }
    }
